"""Shared contracts for V2.111-V2.115 final acceptance artifacts."""
...
PRODUCER_NAME = "workspace_portfolio_final_acceptance"
PRODUCER_VERSION = "1"
...
def artifact_ref(kind: str, filename: str) -> dict[str, str]:
    return {"type": kind, "artifact_ref": f"workspace_portfolio_final_acceptance://portfolio_final_acceptance/{filename}"}
...
def run_id_for(*, workspace_id: str, root_ref: str, input_hashes: dict[str, Any]) -> str:
    return f"v2111-{slug(workspace_id)}-{digest_value({'root_ref': root_ref, 'input_hashes': input_hashes}, length=12)}"
