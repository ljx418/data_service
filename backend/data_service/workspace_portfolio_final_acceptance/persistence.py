"""Persistence helpers for V2.111-V2.115 final acceptance artifacts."""
...
def acceptance_dir(workspace: Path) -> Path:
    return Path(workspace) / "portfolio_final_acceptance"


def artifact_path(workspace: Path, filename: str) -> Path:
    return acceptance_dir(workspace) / filename
