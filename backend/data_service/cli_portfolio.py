"""CLI subcommands for workspace portfolio artifacts."""
...
def add_portfolio_parser(subparsers: argparse._SubParsersAction) -> None:
    parser = subparsers.add_parser("portfolio", help="Build and read workspace portfolio artifacts")
    commands = parser.add_subparsers(dest="portfolio_command", required=True)
    for name in ("scan", "build", "read", "report"):
        ...
        if name == "build":
            command.add_argument("--max-code-projects", type=int, default=1, help="Maximum discovered code projects to build in one bounded run")
...
def run_portfolio_command(args: argparse.Namespace) -> int:
    ...
    service = WorkspacePortfolioService(workspace, workspace_id=str(meta["workspace_id"]))
    if args.portfolio_command == "scan":
        payload = service.scan(root=getattr(args, "root"), limit=int(getattr(args, "limit", 120)))
    elif args.portfolio_command == "build":
        payload = service.build(
            root=getattr(args, "root"),
            limit=int(getattr(args, "limit", 120)),
            max_code_projects=int(getattr(args, "max_code_projects", 1)),
        )
    elif args.portfolio_command == "report":
        payload = service.report()
    else:
        payload = service.read()
    print(json.dumps(public_portfolio_payload(payload), ensure_ascii=False, indent=2))
    return 0
