"""Shared helpers for V2.96-V2.100 automated evidence closure artifacts."""
...
AUTOMATED_EVIDENCE_CLOSURE_SCHEMA_VERSION = "v2.96-100"
...
def artifact_uri(codebase_id: str, section: str, filename: str) -> str:
    return f"automated_evidence_closure://{codebase_id}/{section}/{filename}"
...
def base_artifact(
    *,
    ...
    return {
        "ok": status == "accepted",
        "schema_version": AUTOMATED_EVIDENCE_CLOSURE_SCHEMA_VERSION,
...
def status_summary(rows: list[dict[str, Any]]) -> dict[str, int]:
    statuses = [str(row.get("status") or row.get("e2e_status") or row.get("decision_status") or "needs_review") for row in rows]
    return {
        "accepted_count": sum(1 for status in statuses if status == "accepted"),
...
def worst_status(statuses: list[str]) -> str:
    normalized = [status if status in STATUSES else "needs_review" for status in statuses]
...
def redaction_findings(payload: Any) -> list[dict[str, Any]]:
    ...
    cleaned = (
        ...
        .replace("automated_evidence_closure://", "artifact_ref://")
