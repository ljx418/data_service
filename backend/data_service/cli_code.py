...
from .cli_code_agent_productization import add_agent_productization_parser, agent_productization_tool_payload
from .cli_code_human_agent_deepening import add_human_agent_deepening_parser, human_agent_deepening_tool_payload
...
from .cli_code_automated_evidence_closure import add_automated_evidence_closure_parser, automated_evidence_closure_tool_payload
...
def add_code_parser(subparsers: argparse._SubParsersAction) -> None:
    ...
    add_agent_productization_parser(code_subparsers)
    add_human_agent_deepening_parser(code_subparsers)
    ...
    add_automated_evidence_closure_parser(code_subparsers)
...
def run_code_command(args: argparse.Namespace) -> int:
    if args.code_command not in {"import", "list", "snapshot", "inventory", "symbols", "trace", "overview", "context-pack", "devwiki", "graph", "quality", "architecture", "architecture-intent", "coding-agent", "platform", "agent-productization", "human-agent-deepening", "stabilization-e2e-portal", "external-e2e-portal-delivery", "agent-memory-release", "project-acceptance-hardening", "real-document-acceptance", "real-document-full-corpus-release", "real-acceptance-closure", "automated-evidence-closure", "describe", "archive"}:
    ...
    elif args.code_command == "agent-productization":
        tool_name, payload_args = agent_productization_tool_payload(args)
    elif args.code_command == "human-agent-deepening":
        tool_name, payload_args = human_agent_deepening_tool_payload(args)
    elif args.code_command == "stabilization-e2e-portal":
        tool_name, payload_args = stabilization_e2e_portal_tool_payload(args)
    elif args.code_command == "external-e2e-portal-delivery":
    ...
    elif args.code_command == "real-document-acceptance":
    ...
    elif args.code_command == "automated-evidence-closure":
        tool_name, payload_args = automated_evidence_closure_tool_payload(args)
