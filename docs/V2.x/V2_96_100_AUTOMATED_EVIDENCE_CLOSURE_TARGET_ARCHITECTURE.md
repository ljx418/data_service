...
- V2.96-V2.100 新增实现已落在独立 `automated_evidence_closure` code asset 包内；V2.91-V2.95 的 `real_acceptance_closure` 仅作为上游历史阶段引用。
...
| `backend/data_service/cli_code_real_acceptance_closure.py` | 已开发，作为 V2.91-V2.95 历史阶段入口保留 | 为 code parser 注册 real-acceptance-closure command group |
...
| 目标实体 | 状态 | 当前职责 |
| --- | --- | --- |
| Default CLI Entrypoint Adapter | 已实现并测试 | 让默认 shell CLI 可访问 `python -m data_service code automated-evidence-closure ...` |
| Route A Evidence Automator | 已实现，最终 accepted 仍需人工确认 | 扫描真实资料、生成脱敏检查、截图/headless evidence、人工确认草稿 |
| Quality Decision Workbench | 已实现，最终 accepted 仍需 reviewer decision | 生成风险分级、人类最小决策队列、decision history |
| External Project Path Registry | 已实现，外部路径缺失时保持 `structured_unavailable` | 管理项目路径配置、可读性检查、E2E command refs、unavailable 决议 |
| Release Evidence Aggregator | 已实现，final release 仍非全绿 | 聚合 dependency hygiene、restore smoke、human approval 和上游 accepted artifacts |
...
- MCP tools：`knowledge_code_automated_evidence_closure_*_build/read`。
- CLI：`python -m data_service code automated-evidence-closure <command>`。
- HTTP：`/api/workspaces/{workspace_id}/codebases/{codebase_id}/automated-evidence-closure/...` route family。
...
V2.91-V2.95 的 `real-acceptance-closure` surface 仍作为上游历史阶段能力保留；V2.96-V2.100 的新增实现以 `automated_evidence_closure` 独立包承载。
